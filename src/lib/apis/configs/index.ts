...
export const getLogo = async (token: string) => {
	...
	const res = await fetch(`${WEBUI_API_BASE_URL}/configs/logo`, {
		method: 'GET',
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${token}`
...
export const uploadLogo = async (token: string, formData: FormData) => {
	let error = null;
...
export const resetLogo = async (token: string) => {
	let error = null;
